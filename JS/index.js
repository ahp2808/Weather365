const Api_Link = "	https://api.openweathermap.org/data/2.5/weather?q=";
    ...
    const CBen = (c) => {
      ...
      fetch(Api_Link + c + API_Key + "&units=metric")
        ...
        .then((response) => {
          console.log(response);
          B1.innerHTML = response.main.temp + "&degC";
          B2.innerHTML = response.main.temp_max + "&degC";
          B3.innerHTML = response.main.temp_min + "&degC";
          B4.innerHTML = response.main.humidity + "%";
        })
        .catch((err) => console.error(err));
    };
    CBen(c.value);